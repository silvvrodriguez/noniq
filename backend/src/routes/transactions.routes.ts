...
router.get("/", authenticateToken, async (req: AuthenticatedRequest, res) => {
  ...
  if (
    ...
    (typeof categoryId !== "string" || !z.uuid().safeParse(categoryId).success)
  ) {
    return res.status(400).json({
...
router.patch(
  ...
  async (req: AuthenticatedRequest, res) => {
    ...
    const finalCategoryId = result.data.categoryId ?? transaction.categoryId;

    const finalType = result.data.type ?? transaction.type;
